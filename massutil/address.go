...
func DecodeAddress(addr string, defaultNet *config.Params) (Address, error) {
	...
	case ripemd160.Size + 2: // BindingTarget
		if config.IsPubKeyHashAddrID(netID) {
			return newAddressBindingTarget(decoded, netID)
		}
		return nil, ErrUnknownAddressType
	default:
...
func IsValidBindingTarget(target Address) bool {
	if IsAddressPubKeyHash(target) {
		return true
	}
	_, ok := target.(*AddressBindingTarget)
	return ok
}

// AddressBindingTarget is used since MASS IP2
type AddressBindingTarget struct {
	// [0:ripemd160.Size]	- hash of MASS plot pubkey or chia plot ID
	// [20]					- target type, 0 for MASS, 1 for Chia
	// [21]					- target size, MASS bitlength or chia K
	script [ripemd160.Size + 2]byte

	// net.PubKeyHashAddrID, same with AddressPubKeyHash
	netID byte
}

func NewAddressBindingTarget(serialized []byte, net *config.Params) (*AddressBindingTarget, error) {
	return newAddressBindingTarget(serialized, net.PubKeyHashAddrID)
}

func newAddressBindingTarget(serialized []byte, netID byte) (*AddressBindingTarget, error) {
	if len(serialized) != ripemd160.Size+2 {
		return nil, errors.New("serialized must be 22 bytes")
	}

	switch serialized[20] {
	case byte(0), byte(1):
	default:
		return nil, fmt.Errorf("unknown target type %d", serialized[20])
	}

	// In fact, there is no limit.
	if serialized[21] < 20 || serialized[21] > 200 {
		return nil, fmt.Errorf("unacceptable target size %d", serialized[21])
	}

	addr := &AddressBindingTarget{netID: netID}
	copy(addr.script[:], serialized)
	return addr, nil
}
