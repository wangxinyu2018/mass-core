...
func CreateDB(path string, args ...interface{}) (storage.Storage, error) {
	return newLevelDB(path, true, false)
}

func OpenDB(path string, readonly bool, args ...interface{}) (storage.Storage, error) {
	return newLevelDB(path, false, readonly)
}

func newLevelDB(path string, create, readonly bool) (store storage.Storage, err error) {
	var ldb *leveldb.DB
	...
	if err != nil {
		logging.CPrint(logging.WARN, "failed to open leveldb", logging.LogFormat{
			"path":     path,
			"create":   create,
			"readonly": readonly,
			"err":      err,
	...
	logging.CPrint(logging.INFO, "open leveldb", logging.LogFormat{
		"path":     path,
		"create":   create,
		"readonly": readonly,
	})
