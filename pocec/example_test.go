...
func Example_signMessage() {
	...
	privKey, pubKey := pocec.PrivKeyFromBytes(pocec.S256(), pkBytes)

	// Sign a message using the private key.
	message := "test message"
	messageHash := wire.DoubleHashB([]byte(message))
...
func Example_verifySignature() {
	...
	signature, err := pocec.ParseSignature(sigBytes, pocec.S256())
	if err != nil {
		fmt.Println(err)
		return
	}
...
func Example_encryptMessage() {
	...
	message := "test message"
	ciphertext, err := pocec.Encrypt(pubKey, []byte(message))
	if err != nil {
		fmt.Println(err)
		return
	}
	...
	privKey, _ := pocec.PrivKeyFromBytes(pocec.S256(), pkBytes)

	// Try decrypting and verify if it's the same message.
	plaintext, err := pocec.Decrypt(privKey, ciphertext)
	if err != nil {
		fmt.Println(err)
		return
	}
