...
func (fpk *FaultPubKey) Encode(w io.Writer, mode CodecMode) (n int, err error) { //TODO:
	pb := fpk.ToProto()
...
func (fpk *FaultPubKey) FromProto(pb *wirepb.Punishment) error {
	...
	fpk.PubKey = fpk.Testimony[0].PublicKey()
	return nil
}
...
func (fpk *FaultPubKey) IsValid() error {
	...
	fpkbytes := fpk.PubKey.SerializeUncompressed()
	if !bytes.Equal(fpkbytes, h0.PublicKey().SerializeUncompressed()) ||
		!bytes.Equal(fpkbytes, h1.PublicKey().SerializeUncompressed()) {
		return errFaultPubKeyWrongPubKey
	}
	pass0, err0 := h0.VerifySig()
	if err0 != nil {
		return err0
	}
	pass1, err1 := h1.VerifySig()
	if err1 != nil {
		return err1
	}
	if !(pass0 && pass1) {
