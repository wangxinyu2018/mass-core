...
func TestExportData(t *testing.T) {
	...
	for i, blk := range chain.Blocks() {
		...
		quality, err := proof.VerifiedQuality(pocutil.PubKeyItfHash(pk), pocutil.Hash(blk.Header.Challenge), false, slot, blk.Header.Height)
		if err != nil {
			t.Fatal(err)
		}

		pbProof := wirepb.ProofToProto(blk.Header.Proof)
		...
		poolArr[i-10] = PoolData{
			...
			// ChainCode:  chaincode,
			Proof: Proof{
				X:         hex.EncodeToString(pbProof.X),
				XPrime:    hex.EncodeToString(pbProof.XPrime),
				BitLength: int(pbProof.BitLength),
			},
			Target:      hex.EncodeToString(blk.Header.Target.Bytes()),
			Quality:     hex.EncodeToString(quality.Bytes()),
			QualityData: poc.MustGetDefaultProof(proof).GetHashVal(uint64(blk.Header.Timestamp.Unix())/poc.PoCSlot, blk.Header.Height).String(),
