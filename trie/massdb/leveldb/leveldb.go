...
func NewCustom(file string, namespace string, customize func(options *opt.Options)) (*Database, error) {
	...
	// }
	logging.CPrint(logging.INFO, "Allocated cache and file handles", logging.LogFormat{
		"path":     file,
...
func (db *Database) Close() error {
	...
	if db.quitChan != nil {
		...
		db.quitChan <- errc
		if err := <-errc; err != nil {
			logging.CPrint(logging.ERROR, "Metrics collection failed", logging.LogFormat{"err": err})
		}
		db.quitChan = nil
	}
	return db.db.Close()
...
func (db *Database) NewBatch() massdb.Batch {
	return &batch{
		db: db.db,
		b:  new(leveldb.Batch),
	}
...
func (db *Database) NewIterator(prefix []byte, start []byte) massdb.Iterator {
...
func (db *Database) meter(refresh time.Duration) {
	...
	for i := 1; errc == nil && merr == nil; i++ {
		...
		if err != nil {
			logging.CPrint(logging.ERROR, "Failed to read database stats", logging.LogFormat{"err": err})
			merr = err
			continue
		}
		...
		if len(lines) <= 3 {
			logging.CPrint(logging.ERROR, "Compaction leveldbTable not found")
		...
		for _, line := range lines {
			...
			for idx, counter := range parts[2:] {
				...
				if err != nil {
					logging.CPrint(logging.ERROR, "Compaction entry parsing failed", logging.LogFormat{"err": err})
					merr = err
					continue
				}
		...
		if err != nil {
			logging.CPrint(logging.ERROR, "Failed to read database write delay statistic", logging.LogFormat{"err": err})
			merr = err
			continue
		}
		var (
		...
		if n, err := fmt.Sscanf(writedelay, "DelayN:%d Delay:%s Paused:%t", &delayN, &delayDuration, &paused); n != 3 || err != nil {
			logging.CPrint(logging.ERROR, "Write delay statistic not found")
			merr = err
			continue
		}
		duration, err = time.ParseDuration(delayDuration)
		if err != nil {
			logging.CPrint(logging.ERROR, "Failed to parse delay duration", logging.LogFormat{"err": err})
			merr = err
			continue
		}
		...
		if paused && delayN-delaystats[0] == 0 && duration.Nanoseconds()-delaystats[1] == 0 &&
			...
			logging.CPrint(logging.WARN, "Database compacting, degraded performance")
		...
		if err != nil {
			logging.CPrint(logging.ERROR, "Failed to read database iostats", logging.LogFormat{"err": err})
			merr = err
			continue
		}
		...
		if len(parts) < 2 {
			logging.CPrint(logging.ERROR, "Bad syntax of ioStats", logging.LogFormat{"ioStats": ioStats})
			merr = fmt.Errorf("bad syntax of ioStats %s", ioStats)
			continue
		}
		if n, err := fmt.Sscanf(parts[0], "Read(MB):%f", &nRead); n != 1 || err != nil {
			logging.CPrint(logging.ERROR, "Bad syntax of read entry", logging.LogFormat{"entry": parts[0]})
			merr = err
			continue
		}
		if n, err := fmt.Sscanf(parts[1], "Write(MB):%f", &nWrite); n != 1 || err != nil {
			logging.CPrint(logging.ERROR, "Bad syntax of write entry", logging.LogFormat{"entry": parts[1]})
			merr = err
			continue
		}
		...
		if n, err := fmt.Sscanf(compCount, "MemComp:%d Level0Comp:%d NonLevel0Comp:%d SeekComp:%d", &memComp, &level0Comp, &nonLevel0Comp, &seekComp); n != 4 || err != nil {
			logging.CPrint(logging.ERROR, "Compaction count statistic not found")
			merr = err
			continue
		}
...
func (b *batch) Replay(w massdb.KeyValueWriter) error {
	return b.b.Replay(&replayer{writer: w})
}

// replayer is a small wrapper to implement the correct replay methods.
type replayer struct {
	writer  massdb.KeyValueWriter
	failure error
}
