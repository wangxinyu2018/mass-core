...
import (
	...
	triepb "github.com/wangxinyu2018/mass-core/trie/pb"
)
...
func stackTrieFromPool(db massdb.KeyValueWriter) *StackTrie {
...
type StackTrie struct {
	...
	db        massdb.KeyValueWriter // Pointer to the commit db, can be nil
}
...
func (st *StackTrie) setDb(db massdb.KeyValueWriter) {
	st.db = db
	for _, child := range st.children {
		if child != nil {
			child.setDb(db)
...
func newLeaf(ko int, key, val []byte, db massdb.KeyValueWriter) *StackTrie {
...
func newExt(ko int, key []byte, child *StackTrie, db massdb.KeyValueWriter) *StackTrie {
	st := stackTrieFromPool(db)
	st.nodeType = extNode
	st.keyOffset = ko
	st.key = append(st.key, key[ko:]...)
...
func (st *StackTrie) Update(key, value []byte) {
	if err := st.TryUpdate(key, value); err != nil {
		logging.CPrint(logging.ERROR, fmt.Sprintf("Unhandled trie error: %v", err))
	}
}
...
func (st *StackTrie) hash() {
	...
	case branchNode:
		...
		for i, child := range st.children {
			...
			child.hash()
			if len(child.val) < 32 {
				// nodes[i] = rawNode(child.val) // TODO:
				pb := new(triepb.Node)
				err := proto.Unmarshal(child.val, pb)
				if err != nil {
					panic(err)
				}
				if len(pb.Children) != 2 {
					panic("unexpect child value in branch node")
				}
				n := new(shortNode)
				n.Key = pb.Children[0].Val
				...
				default:
					panic("invalid short node value")
				}
				nodes[i] = n
		...
		if enc, err := encodeNode(n); err != nil {
			panic(err)
		} else {
			if _, err = h.tmp.Write(enc); err != nil {
				panic(err)
			}
		}
	case extNode:
		...
		if len(st.children[0].val) < 32 {
			// valuenode = rawNode(st.children[0].val) // TODO:
			valuenode = valueNode(st.children[0].val)
		} else {
