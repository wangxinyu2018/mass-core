...
func init() {
	massutil.WitPkScriptParseFunc = func(pkScript []byte) (class byte, frozen uint64, witHash [32]byte, boundPkScript []byte) {
		...
		if clazz == WitnessV0ScriptHashTy ||
			...
			if err != nil {
				clazz = NonStandardTy
			} else if clazz == BindingScriptHashTy {
				_, boundPkScript, _ = GetParsedBindingOpcode(pops)
			}
		}
		return byte(clazz), frozen, witHash, boundPkScript
	}
}
...
func GetParsedOpcode(pops []parsedOpcode, class ScriptClass) (uint64, [32]byte, error) {
	...
	case BindingScriptHashTy:
		...
		if len(pops[2].data) != OP_DATA_20 && len(pops[2].data) != OP_DATA_22 {
...
func payToBindingScriptHashScript(scriptHash1 []byte, scriptHash2 []byte) ([]byte, error) {
	if len(scriptHash1) != WitnessV0ScriptHashDataSize ||
		(len(scriptHash2) != OP_DATA_20 && len(scriptHash2) != OP_DATA_22) {
		return nil, ErrWitnessProgramLength
	}
...
func ExtractPkScriptAddrs(pkScript []byte, chainParams *config.Params) (ScriptClass, []massutil.Address, []*btcec.PublicKey, int, error) {
	...
	case BindingScriptHashTy:
		...
		var target massutil.Address
		if len(pops[2].data) == OP_DATA_20 {
			target, err = massutil.NewAddressPubKeyHash(pops[2].data, chainParams)
		} else {
			target, err = massutil.NewAddressBindingTarget(pops[2].data, chainParams)
		}
		if err == nil {
