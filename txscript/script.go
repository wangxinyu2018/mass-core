...
func isWitnessBindingScript(pops []parsedOpcode) bool {
	return len(pops) == 3 &&
		pops[0].opcode.value == OP_0 &&
		pops[1].opcode.value == OP_DATA_32 &&
		(pops[2].opcode.value == OP_DATA_20 || // before mass ip2, pure binding pk hash
			pops[2].opcode.value == OP_DATA_22) // binding pk hash + 1-byte proof type + 1-byte bitlength(k)
}
...
func IsMASSWitnessProgram(script []byte) bool {
	...
	return isMASSWitnessProgram(pops)
}

func isMASSWitnessProgram(pops []parsedOpcode) bool {
	return len(pops) == 3 &&
		isSmallInt(pops[0].opcode) &&
		canonicalPush(pops[1]) && (len(pops[1].data) >= 2 && len(pops[1].data) <= 40) &&
		canonicalPush(pops[2]) && (len(pops[2].data) == 8 || //	lockheight (int64) for staking
		len(pops[2].data) == 20 || // old binding target
		len(pops[2].data) == 22) // new binding target
}
...
func extractWitnessProgramInfo(pops []parsedOpcode) (int, []byte, []parsedOpcode, error) {
	...
	// extract.
	if !isWitnessProgram(pops) && !isMASSWitnessProgram(pops) {
