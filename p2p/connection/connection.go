...
func (c *MConnection) Send(chID byte, msg interface{}) bool {
	...
	channel, ok := c.channelsIdx[chID]
	if !ok {
		logging.CPrint(logging.ERROR, "cannot send bytes due to unknown channel", logging.LogFormat{"chID": chID})
		return false
	}

	if !channel.sendBytes(wire.BinaryBytes(msg)) {
		logging.CPrint(logging.ERROR, "MConnection send failed", logging.LogFormat{"chID": chID, "conn": c, "msg": msg})
		return false
	}
...
func (c *MConnection) flush() {
	if err := c.bufWriter.Flush(); err != nil {
		logging.CPrint(logging.ERROR, "MConnection flush failed", logging.LogFormat{"err": err})
	}
}
...
func (c *MConnection) recvRoutine() {
	...
	for {
		...
		if err != nil {
			if c.IsRunning() {
				logging.CPrint(logging.ERROR, "connection failed @ recvRoutine (reading byte)", logging.LogFormat{"conn": c, "err": err})
				c.conn.Close()
				c.stopForError(err)
			}
			return
		...
		case packetTypeMsg:
			...
			if err != nil {
				if c.IsRunning() {
					logging.CPrint(logging.ERROR, "failed on recvRoutine", logging.LogFormat{"conn": c, "err": err})
					c.stopForError(err)
				}
				return
			}
...
func (c *MConnection) sendMsgPacket() bool {
	...
	if err != nil {
		logging.CPrint(logging.ERROR, "failed to write msgPacket", logging.LogFormat{"err": err})
		c.stopForError(err)
		return true
	}
...
func (c *MConnection) sendRoutine() {
	...
	for {
		...
		if err != nil {
			logging.CPrint(logging.ERROR, "connection failed @ sendRoutine", logging.LogFormat{"conn": c, "err": err})
			c.stopForError(err)
			return
		}
	}
