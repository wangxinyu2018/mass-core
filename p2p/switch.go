...
import (
	...
	"github.com/wangxinyu2018/mass-core/p2p/discover"
	"github.com/wangxinyu2018/mass-core/p2p/trust"
	"github.com/wangxinyu2018/mass-core/version"
...
func NewSwitch(conf *config.Config) (*Switch, error) {
	...
	nodeDB, err := discover.NewNetworkDB(conf, discover.NodeID(pubKey))
	if err != nil {
		return nil, err
	}
	sw.db = nodeDB
	...
	if !conf.P2P.VaultMode {
		var l Listener
		l, listenerStatus = NewDefaultListener(conf)
		sw.AddListener(l)
...
func initDiscover(sw *Switch, port uint16) (*discover.Network, error) {
	...
	if sw.conf.P2P.Seeds == "" {
		return ntab, nil
	}
	nodes := []*discover.Node{}
	for _, seed := range strings.Split(sw.conf.P2P.Seeds, ",") {
...
func (sw *Switch) ensureInitialAddPeers() {
	...
	for _, addr := range sw.conf.P2P.AddPeer {
