...
func ticketToPong(t *ticket, pong *pong) {
	...
	if err != nil {
		logging.CPrint(logging.ERROR, "wireHash err", logging.LogFormat{"err": err})
	}
...
type ticketStore struct {
	...
	queriesSent           map[*Node]map[Hash]sentQuery
}
...
func newTicketStore() *ticketStore {
	return &ticketStore{
		...
		queriesSent:    make(map[*Node]map[Hash]sentQuery),
	}
}
...
func (s *ticketStore) addTopic(topic Topic, register bool) {
	logging.CPrint(logging.DEBUG, "adding discovery topic", logging.LogFormat{"topic": topic, "register": register})
...
func (s *ticketStore) removeRegisterTopic(topic Topic) {
	logging.CPrint(logging.DEBUG, "removing discovery topic", logging.LogFormat{"topic": topic})
	if s.tickets[topic] == nil {
		logging.CPrint(logging.WARN, "removing non-existent discovery topic", logging.LogFormat{"topic": topic})
		return
	}
...
func (s *ticketStore) nextRegisterLookup() (lookupInfo, time.Duration) {
	...
	for len(s.regQueue) > 0 {
		...
		if s.tickets[topic].nextLookup < Now() {
			...
			logging.CPrint(logging.DEBUG, "found discovery topic to register", logging.LogFormat{"topic": topic, "target": next.target, "delay": delay})
			return next, delay
		}
	}
	// No registration topics found or all exhausted, sleep
	delay := 40 * time.Second
	logging.CPrint(logging.DEBUG, "no topic found to register", logging.LogFormat{"delay": delay})
...
func (s *ticketStore) ticketsInWindow(topic Topic) []ticketRef {
	...
	if s.tickets[topic] == nil {
		logging.CPrint(logging.WARN, "listing non-existing discovery tickets", logging.LogFormat{"topic": topic})
		return nil
	}
	...
	logging.CPrint(logging.DEBUG, "retrieved discovery registration tickets", logging.LogFormat{"topic": topic, "from": s.lastBucketFetched, "tickets": len(tickets)})
	return tickets
}
...
func (s *ticketStore) addTicketRef(r ticketRef) {
	...
	if tickets == nil {
		logging.CPrint(logging.WARN, "adding ticket to non-existent topic", logging.LogFormat{"topic": topic})
		return
	}
...
func (s *ticketStore) removeTicketRef(ref ticketRef) {
	logging.CPrint(logging.DEBUG, "removing discovery ticket reference", logging.LogFormat{"node": ref.t.node.ID, "serial": ref.t.serial})
	...
	if tickets == nil {
		logging.CPrint(logging.DEBUG, "removing tickets from unknown topic", logging.LogFormat{"topic": topic})
		return
	}
	...
	if idx == -1 {
		logging.CPrint(logging.FATAL, "idx == -1")
	}
	...
	for i, n := range nodes {
		if i == 0 || (binary.BigEndian.Uint64(n.sha[:8])^binary.BigEndian.Uint64(lookup.target[:8])) < s.radius[lookup.topic].minRadius {
			...
			if s.canQueryTopic(n, lookup.topic) {
				...
				if hash != nil {
					s.addTopicQuery(BytesToHash(hash), n, lookup)
				}
			}
			//}
		}
...
func (s *ticketStore) adjustWithTicket(now AbsTime, targetHash Hash, t *ticket) {
...
func (s *ticketStore) addTicket(localTime AbsTime, pingHash []byte, ticket *ticket) {
	logging.CPrint(logging.DEBUG, "adding discovery ticket", logging.LogFormat{"node": ticket.node.ID, "serial": ticket.serial})
...
func (s *ticketStore) getNodeTicket(node *Node) *ticket {
	if s.nodes[node] == nil {
		logging.CPrint(logging.DEBUG, "retrieving node ticket", logging.LogFormat{"node": node.ID, "serial": nil})
	} else {
		logging.CPrint(logging.DEBUG, "retrieving node ticket", logging.LogFormat{"node": node.ID, "serial": s.nodes[node].serial})
	}
	return s.nodes[node]
}
...
func (s *ticketStore) addTopicQuery(hash Hash, node *Node, lookup lookupInfo) {
	now := Now()
	qq := s.queriesSent[node]
	if qq == nil {
		qq = make(map[Hash]sentQuery)
...
func (s *ticketStore) gotTopicNodes(from *Node, hash Hash, nodes []rpcNode) (timeout bool) {
	now := Now()
...
func (r *topicRadius) getBucketIdx(addrHash Hash) int {
...
func (r *topicRadius) targetForBucket(bucket int) Hash {
...
func (r *topicRadius) isInRadius(addrHash Hash) bool {
...
func (r *topicRadius) chooseLookupBucket(a, b int) int {
	...
	logging.CPrint(logging.FATAL, "should never happen")
...
func (r *topicRadius) recalcRadius() (radius uint64, radiusLookup int) {
	...
	if slopeCross != maxBucket && (minRadBucket <= maxBucket || r.needMoreLookups(maxBucket+lookupWidth, len(r.buckets)-1, maxValue)) {
		for len(r.buckets) <= maxBucket+lookupWidth {
			r.buckets = append(r.buckets, topicRadiusBucket{lookupSent: make(map[Hash]AbsTime)})
		}
...
func (r *topicRadius) adjustWithTicket(now AbsTime, targetHash Hash, t ticketRef) {
...
func (r *topicRadius) adjust(now AbsTime, targetHash, addrHash Hash, inside float64) {
