...
func NewNetworkDB(cfg *config.Config, self NodeID) (NetworkDB, error) {
...
func newNodeDB(cfg *config.Config, version int, self NodeID) (*nodeDB, error) {
	// if path == "" {
...
func newPersistentNodeDB(cfg *config.Config, version int, self NodeID) (*nodeDB, error) {
	path := path.Join(cfg.Datastore.Dir, "discover.db")

	var stor storage.Storage
	_, err := os.Stat(path)
	if os.IsNotExist(err) {
		stor, err = storage.CreateStorage(cfg.Datastore.DBType, path, nil)
	} else {
		stor, err = storage.OpenStorage(cfg.Datastore.DBType, path, false)
	}
	if err != nil {
		return nil, err
	}
