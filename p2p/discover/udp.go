...
func ListenUDP(priv *crypto.PrivKeyEd25519, conn conn, db NetworkDB, netrestrict *netutil.Netlist) (*Network, error) {
	...
	logging.CPrint(logging.INFO, "UDP listener up v5", logging.LogFormat{"net": net.tab.self})
	transport.net = net
...
func (t *udp) sendPacket(toid NodeID, toaddr *net.UDPAddr, ptype byte, req interface{}) (hash []byte, err error) {
	...
	if _, err = t.conn.WriteToUDP(packet, toaddr); err != nil {
		logging.CPrint(logging.INFO, "UDP send failed", logging.LogFormat{"err": err})
	}
	return hash, err
}
...
func (t *udp) readLoop() {
	...
	for {
		...
		if netutil.IsTemporaryError(err) {
			...
			logging.CPrint(logging.DEBUG, "temporary read error", logging.LogFormat{"err": err})
			continue
		} else if err != nil {
			// Shut down the loop for permament errors.
			logging.CPrint(logging.DEBUG, "read error", logging.LogFormat{"err": err})
			return
		}
