...
func (sm *SyncManager) handleTransactionMsg(peer *peer, msg *TransactionMessage) {
	...
	if isOrphan, err := sm.chain.ProcessTx(tx); err != nil && !isOrphan {
		if err == errors.ErrTxAlreadyExists || err == blockchain.ErrDoubleSpend ||
			(!sm.IsCaughtUp() &&
				(err == blockchain.ErrImmatureSpend ||
					err == blockchain.ErrBindingInputMissing ||
					err == blockchain.ErrSequenceNotSatisfied ||
					err == blockchain.ErrPlotPKAlreadyBound || err == blockchain.ErrPlotPKNotBound ||
					err == blockchain.ErrBindingValueNotEnough || err == blockchain.ErrInvalidBindingScript ||
					err == blockchain.ErrFeeForPoolPkCoinbase)) {
			return
		}
