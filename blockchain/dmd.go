...
func (dmd *DoubleMiningDetector) isPubKeyBannedTillHeight(height uint64, pk interfaces.PublicKey) (bool, error) {
...
func recursiveCheckPubKeyBanned(dmd *DoubleMiningDetector, node *BlockNode, nodeChildPubKey interfaces.PublicKey) (bool, error) {
	if node.InMainChain {
		return dmd.isPubKeyBannedTillHeight(node.Height, nodeChildPubKey)
	}

	pubKeyString := hex.EncodeToString(nodeChildPubKey.SerializeCompressed())
	...
	if dmd.banState[*node.Hash] != nil && dmd.banTrackNode[*node.Hash] != nil {
		...
		return recursiveCheckPubKeyBanned(dmd, dmd.banTrackNode[*node.Hash], nodeChildPubKey)
		...
		for cursor := node.Parent; cursor != nil; cursor = cursor.Parent {
			...
			if cursor.InMainChain {
				return dmd.isPubKeyBannedTillHeight(cursor.Height, nodeChildPubKey)
			}
			// check each pk in node banList
			for _, key := range cursor.BannedPublicKeys() {
...
func (dmd *DoubleMiningDetector) isPubKeyBanned(node *BlockNode, pubKey interfaces.PublicKey) (bool, error) {
	dmd.Lock()
	defer dmd.Unlock()
...
func makeAppearedPubKeyKey(pubKey interfaces.PublicKey, height uint64, bl int) string {
...
func (dmd *DoubleMiningDetector) applyBlockNode(node *BlockNode) (bool, *wire.Hash, *wire.Hash) {
	...
	for _, pubKey := range node.BannedPublicKeys() {
		pubKeyString := hex.EncodeToString(pubKey.SerializeCompressed())
		dmd.banState[nodeHash][pubKeyString] = struct{}{}
	}

	mKey := makeAppearedPubKeyKey(node.PublicKey(), node.Height, node.BitLength())
