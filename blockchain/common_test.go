...
func newTestBlockchain(db database.Db, datastoreDir string) (*Blockchain, error) {
	bindingDb, err := rawdb.NewLevelDBDatabase(filepath.Join(datastoreDir, "bindingstate"), 0, 0, "", false)
	if err != nil {
		return nil, err
	}

	chain, err := NewBlockchain(&Config{
		DB:             db,
		StateBindingDb: state.NewDatabase(bindingDb),
		ChainParams:    &config.ChainParams,
		CachePath:      filepath.Join(datastoreDir, BlockCacheFileName),
	})
