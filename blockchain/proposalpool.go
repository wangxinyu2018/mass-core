...
func (p *PunishmentProposal) LessThan(other *PunishmentProposal) bool {
	pType := p.Testimony[0].Proof.Type()
	oType := other.Testimony[0].Proof.Type()
	switch {
	case pType == oType:
...
// Returns proposals with the same bitlength each call.
func (l *punishmentProposalList) slice() []*PunishmentProposal {
	results := make([]*PunishmentProposal, 0, len(l.elements))
	for e := l.list.Front(); e != nil; e = e.Next() {
		proposal := e.Value.(*PunishmentProposal)
		if len(results) == 0 || proposal.BitLength == results[0].BitLength {
...
func (l *punishmentProposalList) remove(pubKey interfaces.PublicKey) {
