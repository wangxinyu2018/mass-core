...
func (chain *Blockchain) connectBlock(node *BlockNode, block *massutil.Block) (err error) {
	...
	bindingState, err := node.ParentBindingState(chain.stateBindingDb)
	if err != nil {
		return err
	}
	if err = chain.addrIndexer.SyncAttachBlock(bindingState, block, txInputStore); err != nil {
		return err
	}
	if forks.EnforceMASSIP0002WarmUp(node.Height) {
		root := bindingState.Hash()
		if root != block.MsgBlock().Header.BindingRoot {
			logging.CPrint(logging.ERROR, "wrong binding state root", logging.LogFormat{
				"height": node.Height,
				"block":  node.Hash,
				"err":    err,
			})
		...
		if err = node.SetBindingState(bindingState); err != nil {
			return err
		}

		commitRoot, err := bindingState.Commit()
		if err != nil {
			node.SetBindingState(nil) // revert
			logging.CPrint(logging.ERROR, "failed to commit binding state", logging.LogFormat{
				"height": node.Height,
				"block":  node.Hash,
				"err":    err,
			})
		...
		if root != commitRoot {
			node.SetBindingState(nil) // revert
			logging.CPrint(logging.ERROR, "wrong commit binding state root", logging.LogFormat{
				"height": node.Height,
				"block":  node.Hash,
				"err":    err,
			})
		...
		if err := chain.submitFaultPubKeyFromHash(hashA, hashB); err == nil {
			logging.CPrint(logging.INFO, "faultPubKey submitted to ProposalPool",
				logging.LogFormat{
					"hashA":      hashA,
					"hashB":      hashB,
					"pubKey":     hex.EncodeToString(node.PublicKey().SerializeCompressed()),
...
func (chain *Blockchain) connectBestChain(node *BlockNode, block *massutil.Block, flags BehaviorFlags) error {
	...
	if node.Parent.Hash.IsEqual(chain.blockTree.bestBlockNode().Hash) {
		...
		if !block.IsImport() {
			...
			err = chain.notifyBlockConnected(block)
			if err != nil {
				return err
			}
		}
