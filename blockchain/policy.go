...
func checkParsePkScriptNew(nextBlockHeight uint64, bindingState state.Trie,
	bindingExistInPool func([]byte) bool, txStore TxStore, txs ...*massutil.Tx) error {
	// To simplify this check, we don't distinguish between old binding and new binding.
	ensureTxInNoBinding := func(tx *massutil.Tx) error {
		for _, txIn := range tx.MsgTx().TxIn {
			txInHash := txIn.PreviousOutPoint.Hash
			txInIndex := txIn.PreviousOutPoint.Index

			if txStore[txInHash].Tx == nil {
				logging.CPrint(logging.INFO, "binding input missing",
					logging.LogFormat{
						"tx":    tx.Hash(),
	...
	enforeIp2WarmUp := forks.EnforceMASSIP0002WarmUp(nextBlockHeight)

	newBindingTargetCache := make(map[string]struct{})
	networkBinding := massutil.ZeroAmount()
	if enforeIp2WarmUp {
		var err error
		if networkBinding, err = GetNetworkBinding(bindingState); err != nil {
			return err
		}
	}
	...
	for _, tx := range txs {
		...
		for j, txOut := range tx.TxOut() {
			...
			case txscript.BindingScriptHashTy:
				hasBinding = true
				if enforeIp2WarmUp {
					if len(psi.BoundPkScript) != txscript.OP_DATA_22 {
						logging.CPrint(logging.ERROR, "invalid binding script length", logging.LogFormat{"length": len(psi.BoundPkScript), "expect": 22})
						return ErrInvalidBindingScript
					}

					// disallow duplicate targets
					if _, ok := newBindingTargetCache[string(psi.BoundPkScript)]; ok {
						logging.CPrint(logging.ERROR, "duplicate binding targets", logging.LogFormat{"tx": tx.Hash(), "output": j})
						return ErrInvalidBindingScript
					}
					newBindingTargetCache[string(psi.BoundPkScript)] = struct{}{}

					if bindingExistInPool != nil {
						if bindingExistInPool(psi.BoundPkScript) {
							logging.CPrint(logging.WARN, "binding already exits in pool", logging.LogFormat{
								"target": hex.EncodeToString(psi.BoundPkScript),
								"tx":     tx.Hash(),
								"output": j,
							})
					...
					if data, err := bindingState.TryGet(psi.BoundPkScript); err != nil {
						logging.CPrint(logging.ERROR, "TryGet error", logging.LogFormat{"err": err})
						return err
					} else if data != nil {
						return ErrPlotPKAlreadyBound
					}

					proofType := poc.ProofType(psi.BoundPkScript[20])
					proofSize := int(psi.BoundPkScript[21])
					if !proofType.EnsureBitLength(proofSize) {
						logging.CPrint(logging.ERROR, "invalid bitlength", logging.LogFormat{"type": proofType, "size": proofSize})
						return ErrInvalidBitLength
					}
					requiredBinding, err := forks.GetRequiredBinding(nextBlockHeight, poc.PlotSize(proofType, proofSize), -1, networkBinding)
					if err != nil {
						logging.CPrint(logging.ERROR, "failed to get required binding price", logging.LogFormat{"err": err})
						return err
					}
					if txOut.Value < requiredBinding.IntValue() {
						return ErrBindingValueNotEnough
					}
				} else {
					if len(psi.BoundPkScript) != txscript.OP_DATA_20 {
						logging.CPrint(logging.ERROR, "invalid binding script length", logging.LogFormat{"length": len(psi.BoundPkScript), "expect": 20})
						return ErrInvalidBindingScript
					}
				}
		...
		if hasBinding && !IsCoinBaseTx(tx.MsgTx()) {
			if err := ensureTxInNoBinding(tx); err != nil {
				return err
			}
		}
	}
...
func checkTransactionStandard(bst state.Trie, tx *massutil.Tx, nextBlockHeight uint64, minRelayTxFee massutil.Amount, txStore TxStore, bindingExistInPool func([]byte) bool) error {
	...
	if !IsFinalizedTransaction(tx, nextBlockHeight, time.Now()) {
	...
	err := checkParsePkScriptNew(nextBlockHeight, bst, bindingExistInPool, txStore, tx)
	if err != nil {
